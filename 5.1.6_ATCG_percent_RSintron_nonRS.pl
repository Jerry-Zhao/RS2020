#! /usr/local/perl -w
# Manually remove RS sites  ==> RSintrons_40AGGT20_nonRS.txt
#
open(INA,"Result/05_motif/RSintrons_40AGGT20_nonRS.txt")||die("Can't open INA:$!\n");
open(OUA,">Result/05_motif/RSintrons_nonRS_20up_ATCG.xls")||die("Can't write OUT:$!\n");
open(OUB,">Result/05_motif/RSintrons_nonRS_20up_percent.xls")||die("Can't write OUT:$!\n");
...
{
 chomp;
 if(/^[\w\:]+\s+[ATCG]{20}([ATCG]{20})(AGGT[ATCG]{4})[ATCG]{16}$/)
 {
  $seq=$1; $a1++;
