...
open(OUT,">Result/06_RSexon/count/$sam.junc_count_5end.txt")||die("Can't write OUT:$!\n");
...
while(<INA>)
{
 ...
 if(/^(chr\w\w?)\:(\d+)\-(\d+)\:([+|-])\s+([\w\.\-]+)\s*$/)
 {
  ...
  if($1 eq $chromosome)
  {
   if($4 eq "+"){$site{$2}="$1:$2-$3:$4:$5";$count{$2}=0;} ## 5' end 
   elsif($4 eq "-"){$site{$3}="$1:$2-$3:$4:$5";$count{$3}=0;} ## 5' end
 ...
 if(/^([\w\.\-\:]+)\t(\d+)\t(\w\w?)\t(\d+)\t255\t(\w+)\t\=\t/)
 {
  ...
  if(($2==99) or ($2==147)) ## left of the pair; first in the sam file 
  {
   while($cigar=~/(\d+)([A-Z])/g)
   {
    ...
    elsif($2 eq "N"){$loci=$start; if(exists $count{$loci}){$count{$loci}++;} $start=($start + 1) + $1 -1;}
  ...
  elsif(($2==163) or ($2==83)) ## gene in plus strand; read in minus strand
  {
   ...
   {
    if(($2 eq "M") or ($2 eq "D")){$start=$start+$1-1 +1;}
    elsif($2 eq "N"){$start=$start+$1-1 +1; $loci=$start+1; if(exists $count{$loci}){$count{$loci}++;}}
    elsif($2 eq "I" or $2 eq "H" or $2 eq "S"){}
    else{print "error CIGAR $cigar\n";}
   }
  }
